...
export function ShareLinkModal({ isOpen, onClose, project }: ShareLinkModalProps) {
  ...
  const [collaborationLink, setCollaborationLink] = useState(`https://logspace.dev/invite/${project.id}?token=abc123xyz`)
  ...
  const handleGenerateNewCollabLink = async () => {
    ...
    try {
      ...
      console.log('Generating new collaboration link for project:', project.id)
      ...
      setCollaborationLink(`https://logspace.dev/invite/${project.id}?token=${newToken}`)
    } catch (error) {
